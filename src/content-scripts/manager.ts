...
const createAreAllMet = (id: RuleID): AreAllMet => {
  const ruleState = state.get(id) as RuleState
...
const enableCondition = (condition: Condition, rule: Rule, areAllMet: AreAllMet) => {
  const callback = (met: boolean) => {
    const ruleState = state.get(rule.id) as RuleState
    ruleState[condition.id].met = met
    const allMet = areAllMet()
    const action = allMet ? rule.actionOn : rule.actionOff
    switch (action.id) {
      case 'scene-turn-on':
        // TODO: for scene-turn-on, data.entityID should be set. express the relation in type defs
        api.postService('scene', 'turn_on', { entity_id: action.data ? action.data.entityID : '' })
        break
    }
  }
  switch (condition.id) {
